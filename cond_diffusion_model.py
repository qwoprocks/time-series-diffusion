...
import cv2
from cond_diffusion_utils import *
...
class Unet(nn.Module):
    def __init__(
        ...
        self.rnn_layer = nn.Sequential(
            nn.LayerNorm(2036),
            nn.LSTM(input_size=2036, hidden_size=256, num_layers=1, batch_first=True),
        )
    ...
    def forward(self, x, time, x_self_cond=None, time_cond=None):
        ...
        if time_cond is not None:
            ...
            x_rnn_cond = self.rnn_proj(x_rnn_cond).unsqueeze(0)

        x = torch.cat((x_rnn_cond, x), dim=1)
